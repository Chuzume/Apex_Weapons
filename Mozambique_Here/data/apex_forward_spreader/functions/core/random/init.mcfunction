#> apex_forward_spreader:core/random/init
#
# 疑似乱数生成処理を初期化します
#
# @within function apex_forward_spreader:core/load
...
# 初期化フラグ
    data modify storage apex_forward_spreader: RNGInit set value true
# ランダムシード取得用Entityを召喚
    summon marker ~ ~ ~ {Tags:["Random"]}
# 負荷軽減用にstorageに入れる
    data modify storage apex_forward_spreader: UUID set from entity @e[type=marker,tag=Random,limit=1]
# シードとして代入
...
# リセット
    data remove storage apex_forward_spreader: UUID
