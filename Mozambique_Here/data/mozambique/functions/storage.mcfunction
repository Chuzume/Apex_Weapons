...
# ウィングマン
    execute if data storage chuz:context Item.Mainhand.tag{ItemName:Wingman} run function mozambique:item/wingman/main
    execute if data storage chuz:context Item.Mainhand.tag{ItemName:Wingman_Reload} run function mozambique:item/wingman/reloading
    execute unless data storage chuz:context Item.Mainhand.tag{ItemName:Wingman_Reload} run scoreboard players reset @s C.Apex_R.Wing

# オフハンドに持てないアイテムが入ったら戻す
    execute if data storage chuz:context Item.Offhand.tag{NoOffhand:true} in overworld run function mozambique:item/no_offhand
