...
# わたす
    ...
    execute if data storage chuz:context {Reload:1} if entity @s[nbt={Item:{tag:{ItemName:Wingman}}}] as @p[tag=Chuz.This] run function mozambique:item/wingman/reload_start
    execute if data storage chuz:context {Reload:1} run kill @s

# 最大時
    execute if data storage chuz:context {Reload:0} in overworld run function mozambique:entity/thrown_item/return
