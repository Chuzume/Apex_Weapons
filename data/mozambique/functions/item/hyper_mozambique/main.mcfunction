...
# 発砲(非スニーク)
    ...
    execute if data storage chuz:context Item.Mainhand.tag{CoolTime:0} run say yo
...
# 見た目
# 通常時
    execute if entity @s run scoreboard players set $CMD Chuz.Temporary 16
# 反動(腰だめ)
    execute if entity @s[tag=!C.Apex_ADSShot] if score @s C.Apex_Delay matches 1..3 run scoreboard players set $CMD Chuz.Temporary 17
# ADS
...
# 反動(ADS)
    execute if entity @s[tag=!C.Apex_NormalShot] if score @s C.Apex_Delay matches 1..3 run scoreboard players set $CMD Chuz.Temporary 19
