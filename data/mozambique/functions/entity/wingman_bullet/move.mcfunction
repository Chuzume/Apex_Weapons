...
# プレイヤーの衝突判定
    execute if entity @s[tag=C.Apex_NoHit,tag=C.Apex_NoHitUser] positioned ~-0.5 ~0.1 ~-0.5 positioned ~0.3 ~0.0 ~0.3 if entity @e[dx=0,type=!#mozambique:unhurtable,type=!ender_dragon,sort=nearest,limit=1] at @s positioned ~-0.5 ~-0.4 ~-0.5 positioned ~-0.3 ~-0.3 ~-0.3 if entity @e[dx=0,type=!#mozambique:unhurtable,sort=nearest,limit=1] at @s run function mozambique:entity/wingman_bullet/hit

# 対ドラゴン
    execute at @s at @e[type=minecraft:ender_dragon,team=!null,distance=..6,sort=nearest,limit=1] run function mozambique:entity/wingman_bullet/hit

# 壁の衝突判定
    execute unless block ~ ~ ~ #mozambique:no_collision run function mozambique:entity/wingman_bullet/break
    
# 再帰
    execute if entity @s[scores={C.Apex_Recursion=1..,C.Apex_Range=1..}] at @s run function mozambique:entity/wingman_bullet/move
